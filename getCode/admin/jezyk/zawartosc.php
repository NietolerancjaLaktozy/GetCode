<?php
		...
		if($polaczenie->connect_errno!=0) //instrukcja warunkowa. Skrypt jest wykonywany tylko jeśli ustanowiono polączenie z bazą danych
		{
			...
			$sql = "SELECT * FROM jezyki";  //zapytanie sql do pobrania akapitu tekstu
			
			echo '<table id="admintab">';
			echo '<tr class="linijka">
				<td class="kolumna">ID</td>
				<td class="kolumna">nazwa</td>
				<td class="kolumna">logo</td>
				<td class="kolumna">opis</td>
				<td class="kolumna">historia</td>
				</tr>';
			if($rezultat=@$polaczenie->query($sql)){			//obsłużenie zapytania SQL
				...
				$nazwa=$wiersz['nazwa'];
				$logo=$wiersz['logo_jezyka'];
				$opis=$wiersz['opis'];
				$historia=$wiersz['historia'];
				echo '<tr class="linijka">
				<td class="kolumna">'.$id.'</td>
				<td class="kolumna">'.$nazwa.'</td>
				<td class="kolumna">'.$logo.'</td>
